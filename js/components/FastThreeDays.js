...
class FastThreeDays extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <div><h3>Fast 3 days order list  </h3></div>
        <div>
...
export default Relay.createContainer(FastThreeDays, {
  initialVariables: {
    first: pageSize,
    last: pageSize,
    after: null,
  ...
  fragments: {
    viewer: () => Relay.QL`
      fragment on User {
        next: fastThreeDays(first: $first, after: $after) @include(if: $next) {
          edges {
            cursor,
            node {
              id,
        ...
        prev: fastThreeDays(last: $last, before: $before) @include(if: $prev) {
          edges {
            cursor,
            node {
              id,
